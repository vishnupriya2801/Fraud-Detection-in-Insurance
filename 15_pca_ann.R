...
table(transformed$FraudFound_P)
...
training_set_pan<- splits[[1]]
validation_set_pan <- splits[[2]]
test_set_pan <- splits[[3]]
...
hyper_params <- list(activation = activation_opt,
                     hidden=hidden)
grid_m <- h2o.grid("deeplearning", x = 2:6, y = 'FraudFound_P',
                   grid_id = "grid20",
                   training_frame = as.h2o(training_set_pan),
                   validation_frame=as.h2o(validation_set_pan),
...
model = h2o.deeplearning(x=2:6,y = "FraudFound_P",
                         training_frame = as.h2o(training_set_pan),
                         validation_frame = as.h2o(validation_set_pan),
                         nfolds =10,
...
y_pred = h2o.predict(model, newdata = as.h2o(test_set_pan[-1]))
y_pred.R=as.data.frame(y_pred)
test_set_pan.R=as.data.frame(test_set_pan)
head(y_pred.R)

cm_d=confusionMatrix(y_pred.R[,1],test_set_pan.R[,1],positive = "1",dnn=c("pred","actual"))
cm_d
